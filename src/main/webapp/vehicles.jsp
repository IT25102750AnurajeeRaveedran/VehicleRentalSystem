...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hero { background: linear-gradient(160deg, #0f0f1a 0%, #1a1428 60%, #0a0a0f 100%); padding: 64px 48px 48px; border-bottom: 1px solid var(--border); position: relative; overflow: hidden; }
    .hero::after { content: ''; position: absolute; top: -60px; right: -60px; width: 400px; height: 400px; border-radius: 50%; background: radial-gradient(circle, #f0c04018 0%, transparent 70%); pointer-events: none; }
    ...
    .btn-rent { flex: 1; padding: 10px; background: var(--accent); color: #000; border-radius: 8px; font-size: 13px; font-weight: 700; text-decoration: none; text-align: center; transition: all 0.15s; border: none; cursor: pointer; }
    .btn-rent:hover { background: #ffd060; }
    ...
    .btn-disabled { flex: 1; padding: 10px; background: #1a1a24; border: 1px solid var(--border); color: var(--muted); border-radius: 8px; font-size: 13px; text-align: center; opacity: 0.5; cursor: not-allowed; }
  ...
  <%
    if (vehicles != null && !vehicles.isEmpty()) {
      ...
      for (Vehicle v : vehicles) {
        String img; String emoji; String t = v.getType().toLowerCase();
        if (t.equals("van")) { img = "https://images.unsplash.com/photo-1543508282-6319a3e2621f?w=600&q=80"; emoji = "🚐"; }
        else if (t.equals("suv")) { img = "https://images.unsplash.com/photo-1519641471654-76ce0107ad1b?w=600&q=80"; emoji = "🚙"; }
        else if (t.equals("bike")) { img = "https://images.unsplash.com/photo-1558981806-ec527fa84c39?w=600&q=80"; emoji = "🏍️"; }
        else if (t.equals("bus")) { img = "https://images.unsplash.com/photo-1570125909232-eb263c188f7e?w=600&q=80"; emoji = "🚌"; }
        else { img = carImgs[carIndex % carImgs.length]; carIndex++; emoji = "🚗"; }
  %>
    <div class="vehicle-card">
      <div class="car-img">
        <img src="<%= img %>" alt="<%= v.getBrand() %> <%= v.getModel() %>" loading="lazy"
      ...
      <div class="card-body">
        ...
        <div class="card-actions">
          <% if (v.isAvailable()) { %>
            <a href="rentVehicle?vehicleId=<%= v.getVehicleId() %>&brand=<%= v.getBrand() %>&model=<%= v.getModel() %>&price=<%= v.getPricePerDay() %>" class="btn-rent">🚗 Rent</a>
          <% } else { %>
            <span class="btn-disabled">🚫 Rented Out</span>
          <% } %>
